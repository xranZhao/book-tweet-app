...
async function runDeepReport() {
  ...
  $('#main').innerHTML = `
    <div class="screen">
      ...
      <div class="loading" style="padding:20px 16px;">
        <p id="elapsed-text">⏱ 已等待 0 秒</p>
        <p style="font-size:12px;color:var(--text-lighter);">Pro 模型 · 结构化深度分析 · 约 60-120 秒</p>
      </div>
    </div>`;
  ...
  const angleFocus = {
    cp: '聚焦 CP 感情线：两人关系演变、糖点/虐点分布、感情线节奏、推文切入角度。',
    family: '聚焦家庭/亲情线：核心亲情关系、关键家庭场景、亲情线的温度与冲突、推文切入角度。',
    plot: '聚焦剧情与世界观：核心剧情脉络、世界观/设定亮点、叙事节奏、推文切入角度。',
    general: '综合角度：兼顾 CP 感情线、亲情/家庭线、剧情世界观、人物成长弧光。',
  };

  const prompt = `你是一位专业的小说推文编辑。请阅读以下小说全文，针对「${angleLabels[angle] || '综合'}」角度输出一份深度结构化阅读报告。

${angleFocus[angle] || angleFocus.general}

报告要求（1200-1800字，关键场景必须带原文引用）：

1. 基础信息
   - 原作/书名、作者、估算字数、分级、CP、标签（3-6个）
2. 核心看点（3-5条）
   - 每条 = 一句判断 + 一句原文/情节支撑
3. 关键场景（至少3个）
   - 每个场景 = 小标题 + 原文引用（30-80字）+ 这个场景为什么重要
4. 切入建议（3个推文角度）
   - 每个角度说明：适合哪种风格（克制/平衡/社媒）、标题方向、重点写哪个场景
5. 避雷预警
   - 列出可能的雷点、严重程度、不适合哪些读者

输出格式：严格使用以下 Markdown 模板填空，不要省略任何板块，不要输出模板以外的总结。
...
${text}`;
  ...
  try {
    const res = await deepSeekChat(CONFIG.WRITE_MODEL, prompt, 6000);
    clearInterval(timerInterval);
