...
SRC_DIR = os.path.join(OUT_DIR, '原md')
...
def single_render(md_path, override_title=None, date_str=None):
    ...
    write_html(md_path, md_text, html, override_title)
...
def batch_render():
    ...
    for md_path in md_files:
        md_text = read_md(md_path)
        html = render(md_text)
        write_html(md_path, md_text, html)
...
def build_export_name(md_text, override_title=None, base_name=None, ext='html'):
    ...
    filename = '_'.join(safe_parts) if safe_parts else (base_name or '排版')
    return f'{filename}.{ext}'


def write_html(md_path, md_text, html, override_title=None):
    base = os.path.splitext(os.path.basename(md_path))[0]
    out_name = build_export_name(md_text, override_title=override_title, base_name=base, ext='html')
